...
func BindGidWithReqId(reqId string) {
	if gIdWithReqId == nil {
		return
	}
	b := make([]byte, 64)
	...
	gIdWithReqId.Store(string(b), reqId)
}
...
func GetPrefix(skip int) *string {
	...
	buf.WriteString(fmt.Sprintf("gid-%d ", n))
	if gIdWithReqId != nil {
		reqId, ok := gIdWithReqId.Load(string(b))
		if ok && reqId != "" {
			buf.WriteString(fmt.Sprintf("reqid-%s ", reqId))
		}
	}
